...
# We use the gradient scaler to scale the gradients
...
def main():
    ...
    # Step 1: Create the gradient scaler
    scaler = GradScaler()

    # Log with tensorboard
    writer = SummaryWriter(comment="amp_training")
    ...
    for epoch in trange(args.epochs, desc="Epoch"):
        ...
        for batch_idx, (data, target) in enumerate(training_progress_bar):
            ...
            # Compute output
            # Step 2: Simply wrap the forward pass in a `with autocast():` block
            # to enable mixed precision
            with torch.cuda.amp.autocast():
                output = model(data)
                loss = criterion(output, target)

            # Step 3: Scale the loss and call backward()
            # IMPORTANT: It is not advisable to call backward() on the loss in mixed precision
            scaler.scale(loss).backward()

            # Step 4: Call scaler.step(optimizer) to unscale the gradients and update the weights
            scaler.step(optimizer)

            # Step 5: Call scaler.update() to update the scale for next iteration
            scaler.update()
